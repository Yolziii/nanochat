...
### Older public checkpoints

Some older public checkpoints (for example `karpathy/nanochat-d32`) predate newer architecture features in current `master`. To load them in the CLI, use the explicit compatibility flag:

```bash
python -m scripts.chat_cli -g d32 -s 650 --checkpoint-compat legacy-d32
```
